...
router.post("/aksiya", (req, res) => {
    ...
    pool.query('INSERT INTO aksiya (title,image,description,start_day,end_day) VALUES ($1,$2,$3,$4) RETURNING *',
...
router.put("/aksiya/:id", (req, res) => {
    ...
    pool.query(
        'UPDATE aksiya SET title=$1,description=$2,image=$3,start_day=$4,end_day=$5, WHERE id = $6',
        [body.title,body.description,imgName,body.start_day,body.end_day,id ],
        (err, result) => {
            if (err) {
                res.status(400).send(err)
            } else {
